    ...
    fetch("https://api-football-v1.p.rapidapi.com/v3/fixtures?date="+today, {
      ...
      .then(result => {
        ...
        rMatches["response"].forEach( (e) => {
          this.matches.push({id : e["fixture"]["id"], dateTime : new Date(e["fixture"]["date"]), shortStatus : e["fixture"]["status"]["short"],
            longStatus :  e["fixture"]["status"]["long"] , elapsed : e["fixture"]["status"]["elapsed"] , leagueid : e["league"]["id"],
            idHome: e["teams"]["home"]["name"] , idAway : e["teams"]["away"]["name"], goalsHome : e["goals"]["home"], goalsAway : e["goals"]["away"] });
  ...
  /*getLiveMatches() {
    ...
    fetch("https://api-football-v1.p.rapidapi.com/v3/fixtures?live=all", {
      ...
      .then(result => {
        // Do things with result
        const rMatches = JSON.parse(result);
        this.liveMatches = rMatches["response"];
      });
